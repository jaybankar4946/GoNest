...
export default function AdminPage() {
  ...
  const moderate = async (id:string, status:string, vl:string, rej?:string) => {
    setBusy(true);
    await adminModerate(id, status, vl, rej);
    setQueue(p=>p.filter(l=>l.id!==id));
    setListings(p=>p.map(l=>l.id===id?{...l,status,verification_level:vl}:l));
    setBusy(false);
  };
    ...
    <>
      ...
      <main style={{maxWidth:1100,margin:'0 auto',padding:'40px 24px 80px'}}>
        ...
        {stats && (
          <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:10,marginBottom:32}}>
            {[
              ...
              {label:'Pending review', value:stats.pendingReview, warn:true},
              {label:'Total leads',    value:stats.totalLeads},
              {label:'Total visits',   value:stats.totalVisits},
              {label:'Total users',    value:stats.totalUsers},
            ].map(s=>(
              <div key={s.label} style={{background:s.warn&&s.value>0?'#FEF2F2':'#F7F7F7',borderRadius:12,padding:'14px 16px'}}>
                <div style={{fontSize:22,fontWeight:700,color:s.warn&&s.value>0?'#DC2626':'#111'}}>{s.value}</div>
        ...
        <div style={{display:'flex',gap:4,marginBottom:28,flexWrap:'wrap'}}>
          {TABS.map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={tabS(t)}>
              {t==='queue'?`Review queue (${queue.length})`:t.charAt(0).toUpperCase()+t.slice(1)}
            </button>
          ))}
        </div>
              ...
              return (
                <div key={l.id} style={{padding:16,border:'1px solid #E5E5E5',borderRadius:12}}>
                  ...
                  <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
                    ...
                    <input placeholder="Rejection reason (required to reject)" value={reason[l.id]??''} onChange={e=>setReason(p=>({...p,[l.id]:e.target.value}))} style={{...inp,width:260}} />
              ...
              return (
                <div key={l.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 14px',border:'1px solid #E5E5E5',borderRadius:10}}>
                  <div style={{flex:1,minWidth:0}}>
                    <p style={{fontSize:13,fontWeight:600,color:'#111',marginBottom:1}} className="line-clamp-1">{l.title}</p>
                    <p style={{fontSize:12,color:'#6B6B6B'}}>{city} · {formatPrice(l.price,l.purpose)} · {l.view_count} views</p>
                  </div>
                  <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
                    <span style={SB(SC[l.status]??'#6B6B6B')}>{l.status.replace('_',' ')}</span>
                    <button onClick={()=>adminToggleFeatured(l.id,!l.featured).then(()=>setListings(p=>p.map(x=>x.id===l.id?{...x,featured:!x.featured}:x)))}
                      style={{...btn(l.featured?'#6B6B6B':'#2563EB'),fontSize:11}}>
                      {l.featured?'Unfeature':'Feature'}
                    </button>
                    {l.status==='pending_review' && (
                      <button onClick={()=>moderate(l.id,'active','verified')} disabled={busy} style={{...btn('#16A34A'),fontSize:11}}>Approve</button>
                    )}
                  </div>
                </div>
              );
            ...
            {brokers.map((b:any)=>(
              <div key={b.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',border:'1px solid #E5E5E5',borderRadius:12}}>
                <div>
                  ...
                  <p style={{fontSize:12,color:'#6B6B6B'}}>{b.email??b.id.slice(0,8)} · {b.role} · {b.agent_verified?'✓ Verified':'Unverified'}</p>
                  {b.rera_number && <p style={{fontSize:11,color:'#9B9B9B'}}>RERA: {b.rera_number}</p>}
                </div>
                <div style={{display:'flex',gap:8,alignItems:'center'}}>
                  ...
                  <select value={b.role} onChange={e=>adminSetRole(b.id,e.target.value).then(()=>adminGetBrokers().then(setBrokers))}
                    style={inp}>
            ...
            {leads.map((l:any)=>(
              <div key={l.id} style={{padding:'12px 14px',border:'1px solid #E5E5E5',borderRadius:10}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                  <p style={{fontSize:13,fontWeight:600,color:'#111'}}>{l.buyer_name} — {l.buyer_phone}</p>
                  <div style={{display:'flex',gap:6,alignItems:'center'}}>
                    <span style={SB(SC[l.status]??'#6B6B6B')}>{l.status}</span>
                    <span style={{fontSize:11,color:'#9B9B9B'}}>{timeAgo(l.created_at)}</span>
                  </div>
                </div>
                {l.message && <p style={{fontSize:12,color:'#3D3D3D'}}>{l.message}</p>}
                <p style={{fontSize:11,color:'#9B9B9B',marginTop:4}}>{l.listing?.title} · {(l.listing?.city as any)?.name}</p>
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            ...
            {visits.map((v:any)=>(
              <div key={v.id} style={{padding:'12px 14px',border:'1px solid #E5E5E5',borderRadius:10}}>
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                  <p style={{fontSize:13,fontWeight:600,color:'#111'}}>{v.requester_name} — {v.requester_phone}</p>
                  <span style={SB(SC[v.status]??'#6B6B6B')}>{v.status}</span>
                </div>
                <p style={{fontSize:12,color:'#3D3D3D'}}>{v.slot_date} at {v.slot_time}</p>
                <p style={{fontSize:11,color:'#9B9B9B',marginTop:4}}>{v.listing?.title} · {(v.listing?.city as any)?.name}</p>
              </div>
            ))}
          </div>
        )}
