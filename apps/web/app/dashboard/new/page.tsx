...
export default function NewListingPage() {
  ...
  const[f,setF]=useState({purpose:'sale',property_type:'apartment',title:'',description:'',bedrooms:1,bathrooms:1,balconies:0,sqft:'',furnishing:'',facing:'',city_id:'',locality_id:'',address_line:'',landmark:'',price:'',maintenance_monthly:'',security_deposit:'',price_negotiable:false,brokerage:'none'});
      ...
      <div style={{maxWidth:560,margin:'0 auto',padding:'40px 24px 80px'}}>
        ...
        {step===3&&(
          <div style={{display:'flex',flexDirection:'column',gap:18}}>
            ...
            {images.length>0&&<div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8}}>{images.map((file,i)=><div key={i} style={{position:'relative',borderRadius:8,overflow:'hidden',aspectRatio:'1/1',background:'#F0F0F0'}}><img src={URL.createObjectURL(file)} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>{i===0&&<span style={{position:'absolute',bottom:4,left:4,fontSize:10,fontWeight:600,color:'#fff',background:'#111',padding:'2px 6px',borderRadius:9999}}>Cover</span>}<button onClick={()=>setImages(p=>p.filter((_,j)=>j!==i))} style={{position:'absolute',top:4,right:4,width:20,height:20,borderRadius:'50%',background:'#fff',display:'flex',alignItems:'center',justifyContent:'center',border:'none',cursor:'pointer'}}><X size={11}/></button></div>)}</div>}
          </div>
        )}
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginTop:36}}>
          ...
          {step<STEPS.length-1
            ?<button onClick={()=>canNext()&&setStep(i=>i+1)} disabled={!canNext()} style={{display:'flex',alignItems:'center',gap:6,padding:'10px 22px',borderRadius:9999,fontSize:13,fontWeight:600,color:'#fff',background:'#111',border:'none',opacity:canNext()?1:0.4,cursor:canNext()?'pointer':'not-allowed'}}>Next <ChevronRight size={15}/></button>
            :<button onClick={submit} disabled={saving} style={{display:'flex',alignItems:'center',gap:6,padding:'10px 22px',borderRadius:9999,fontSize:13,fontWeight:600,color:'#fff',background:'#111',border:'none',opacity:saving?0.6:1,cursor:'pointer'}}>{saving?'Submitting…':<><Check size={15}/>Submit for review</>}</button>}
        </div>
      </div>
    </div>
  );
