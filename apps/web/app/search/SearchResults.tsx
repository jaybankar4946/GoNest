...
export function SearchResults({ cities, init }: { cities: any[]; init: Record<string,string> }) {
  const [results,setResults]=useState<any[]>([]);
  const [total,setTotal]=useState(0);
  const [loading,setLoading]=useState(true);
  const [f,setF]=useState({q:init.q??'',purpose:init.purpose??'',cityId:init.city??'',beds:init.beds??'',type:init.type??'',sort:(init.sort??'newest') as any});
  ...
  return (
    <main style={{maxWidth:1200,margin:'0 auto',padding:'32px 24px'}}>
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:28,alignItems:'center'}}>
        ...
        <select style={S} value={f.sort} onChange={e=>upd('sort',e.target.value)}>
          <option value="newest">Newest</option><option value="price_asc">Price ↑</option>
          <option value="price_desc">Price ↓</option><option value="popular">Popular</option>
        </select>
        <span style={{marginLeft:'auto',fontSize:13,color:'#6B6B6B'}}>{loading?'…':`${total} ${total===1?'property':'properties'}`}</span>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'36px 20px'}}>
        {results.map((l:any)=><PropertyCard key={l.id} listing={l}/>)}
      </div>
    </main>
  );
}
