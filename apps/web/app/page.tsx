      ...
      <main>
        ...
        <section style={{padding:'80px 24px 56px',textAlign:'center'}}>
          <div style={{maxWidth:560,margin:'0 auto'}}>
            <h1 style={{fontFamily:'var(--font-display)',fontSize:'clamp(28px,5vw,40px)',fontWeight:800,color:'#111',letterSpacing:'-0.03em',lineHeight:1.15,marginBottom:10}}>
              Find your next home.
            </h1>
            ...
            {cities.length > 0 && (
              <div style={{display:'flex',justifyContent:'center',flexWrap:'wrap',gap:8,marginTop:14}}>
                {cities.slice(0,5).map(c=>(
                  <Link key={c.id} href={`/search?city=${c.id}`} className="hover:border-[var(--primary)] hover:text-[var(--primary)]"
                    style={{padding:'5px 14px',borderRadius:9999,fontSize:12,color:'#6B6B6B',border:'1px solid #E5E5E5',transition:'color .15s,border-color .15s'}}>
                    {c.name}
                  </Link>
                ))}
              </div>
        ...
        {featured.length > 0 && (
          <section style={{maxWidth:1200,margin:'0 auto',padding:'0 24px 72px'}}>
            <div style={{display:'flex',alignItems:'baseline',justifyContent:'space-between',marginBottom:24}}>
              <h2 style={{fontFamily:'var(--font-display)',fontSize:20,fontWeight:700,color:'#111',letterSpacing:'-0.02em'}}>Featured Properties</h2>
              <Link href="/search" style={{fontSize:13,color:'var(--primary)',fontWeight:500}}>View all</Link>
            </div>
        ...
        {/* Categories */}
        <section style={{background:'var(--gray-1)',padding:'52px 24px'}}>
          <div style={{maxWidth:1200,margin:'0 auto'}}>
            <h2 style={{fontFamily:'var(--font-display)',fontSize:20,fontWeight:700,color:'#111',letterSpacing:'-0.02em',marginBottom:20}}>Explore by type</h2>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))',gap:10}}>
              {[['Apartments','apartment'],['Villas','villa'],['Houses','house'],['Plots','plot'],['Commercial','commercial']].map(([label,type])=>(
                <Link key={type} href={`/search?type=${type}`} className="hover:border-[var(--primary)]"
                  style={{display:'block',background:'#fff',border:'1px solid #E5E5E5',borderRadius:'var(--radius-lg)',padding:'18px 16px',fontSize:14,fontWeight:500,color:'#111',transition:'border-color .15s'}}>
                  {label}
                </Link>
              ))}
            </div>
        ...
        <section style={{maxWidth:1200,margin:'0 auto',padding:'56px 24px 80px'}}>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:40}}>
            {[
              ...
              <div key={t}>
                <div style={{width:36,height:36,borderRadius:'50%',background:'var(--gray-1)',display:'flex',alignItems:'center',justifyContent:'center',marginBottom:10}}>
                  <span style={{width:8,height:8,borderRadius:'50%',background:'var(--primary)'}}/>
                </div>
                <h3 style={{fontFamily:'var(--font-display)',fontSize:14,fontWeight:600,color:'#111',marginBottom:6}}>{t}</h3>
