  ...
  if (isSignUp && step==='role') return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
      <div style={{width:'100%',maxWidth:400}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {ROLES.map(r=>(
            <button key={r.value} onClick={()=>{setRole(r.value);setStep('form');}}
              style={{display:'flex',alignItems:'center',gap:14,padding:'14px 16px',borderRadius:12,border:'1px solid #E5E5E5',textAlign:'left',background:'#fff',cursor:'pointer'}}>
        ...
        <button onClick={()=>{setIsSignUp(false);setStep('role');}} style={{width:'100%',marginTop:20,fontSize:13,color:'#6B6B6B',textAlign:'center'}}>
      ...
      <div style={{width:'100%',maxWidth:360}}>
        ...
        <form onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:10}}>
          ...
          <div style={{position:'relative'}}>
            <Mail size={15} style={{position:'absolute',left:14,top:'50%',transform:'translateY(-50%)',color:'#9B9B9B',pointerEvents:'none'}} />
          ...
          {mode==='password'&&(
            <div style={{position:'relative'}}>
              ...
              <input required type="password" placeholder="Password" value={password} onChange={e=>setPass(e.target.value)} style={{...inp,paddingLeft:38}} />
            </div>
          )}
          {msg&&(
        ...
        <button onClick={()=>{setMode(m=>m==='password'?'magic':'password');setMsg(null);}}
          style={{width:'100%',padding:'11px',borderRadius:9999,fontSize:13,fontWeight:500,color:'#111',border:'1px solid #111',background:'#fff',cursor:'pointer'}}>
