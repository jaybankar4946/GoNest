...
export default function SavedPage() {
  ...
  useEffect(()=>{
    ...
    Promise.all([getSavedListings(user.id),getSavedIds(user.id)]).then(([ls,ids])=>{setListings(ls);setSavedIds(ids);}).finally(()=>setLoading(false));
  },[user]);
      ...
      <main style={{maxWidth:1200,margin:'0 auto',padding:'40px 24px 80px'}}>
        ...
        :listings.length===0?(
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'36px 20px'}}>
            {listings.map((l:any)=><PropertyCard key={l.id} listing={l} isSaved={savedIds.has(l.id)} onToggle={toggle}/>)}
          </div>
        )}
      </main>
      <Footer/>
