    ...
    <nav style={{position:'sticky',top:0,zIndex:50,background:'#fff',borderBottom:'1px solid #E5E5E5'}}>
      <div style={{maxWidth:1200,margin:'0 auto',padding:'0 24px',display:'flex',alignItems:'center',height:64,gap:4}}>
        ...
        <div style={{display:'flex',gap:2,flex:1}}>
          {LINKS.map(l=>(
            <Link key={l.href} href={l.href} style={{padding:'6px 14px',borderRadius:9999,fontSize:14,fontWeight:active(l.href)?600:400,color:active(l.href)?'#111':'#6B6B6B',background:active(l.href)?'#F7F7F7':'transparent'}}>
              {l.label}
            </Link>
          ))}
